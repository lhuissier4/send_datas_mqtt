"""Charge les episodes d'alerte (datas/gold/influxdb_alerte.csv) dans
InfluxDB (base `sensor_live`), table `alerte`.
...
Chaque ligne du CSV est un episode d'alerte (debut_alerte, fin_alerte,
id_alerte, id_machine). Le CSV est lu par blocs (`ALERTE_CHUNK_SIZE`
lignes) pour ne jamais charger tout le fichier en memoire, et chaque bloc
est envoye en un seul write groupe vers InfluxDB.

debut_alerte est le timestamp du point (le moment ou l'episode commence) ;
fin_alerte est conserve comme field en epoch nanosecondes (pas une simple
...
ALERTE_CSV_PATH = os.getenv("ALERTE_CSV_PATH", "./datas/gold/influxdb_alerte.csv")
ALERTE_CHUNK_SIZE = int(os.getenv("ALERTE_CHUNK_SIZE", "20000"))

MEASUREMENT = "alerte"
TAG_COLUMNS = ["id_machine", "id_alerte"]
FIELD_COLUMN = "fin_alerte"
TIME_COLUMN = "debut_alerte"
...
def main() -> None:
    ...
    print(
        ...
        f"(table {MEASUREMENT}, blocs de {ALERTE_CHUNK_SIZE} lignes)",
        flush=True,
    )
