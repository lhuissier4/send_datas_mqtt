"""Construit `datas/gold/postgres_machine_secteur_historique.csv`
(historique des secteurs par machine) a partir de
`datas/silver/dataset_brut.csv`.

Script autonome et memoise : si le csv de sortie existe deja, il est relu
tel quel (aucun recalcul) - cf. `ensure_gold_csv` dans `gold/utils.py` et
`openspec/changes/split-gold-datas-per-dataset/design.md`.
"""

from pathlib import Path

import pandas as pd

from gold.utils import build_machine_secteur_historique_dataframe, ensure_gold_csv, name_csv_file

PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent.parent
SILVER_DATASET_BRUT = PROJECT_ROOT / "datas/silver/dataset_brut.csv"
GOLD_DIR = PROJECT_ROOT / "datas/gold"
OUTPUT_PATH = Path(
    name_csv_file(
        folder_path=GOLD_DIR, filename="machine_secteur_historique", extension=".csv", type_dst="postgres"
    )
)


def build(output_path: Path = OUTPUT_PATH, source_csv: Path = SILVER_DATASET_BRUT) -> pd.DataFrame:
    def _compute() -> pd.DataFrame:
        df_simule = pd.read_csv(source_csv)
        df_historique = build_machine_secteur_historique_dataframe(df_simule)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        df_historique.to_csv(output_path, index=False, encoding="utf-8")
        return df_historique

    return ensure_gold_csv(output_path, _compute)


if __name__ == "__main__":
    build()
