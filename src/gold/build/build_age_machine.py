"""Construit `datas/gold/postgres_age_machine.csv` (age et premier
timestamp par machine) a partir de `datas/silver/dataset_brut.csv`.
...
from gold.utils import build_machine_age_dataframe, ensure_gold_csv, name_csv_file
...
def build(output_path: Path = OUTPUT_PATH, source_csv: Path = SILVER_DATASET_BRUT) -> pd.DataFrame:
    def _compute() -> pd.DataFrame:
        ...
        df_age.to_csv(output_path, index=False, encoding="utf-8")
        return df_age
